...
def load_data() -> tuple[
    ...
    boot_nw_df = pd.read_csv(boot_nw_path) if boot_nw_path.exists() else pd.DataFrame()
    mv_nw_path = FIGURES_DIR / "mv_nw_results.csv"
    mv_nw_df = pd.read_csv(mv_nw_path) if mv_nw_path.exists() else pd.DataFrame()
...
"""
    for r in rows:
        latex += (
            f"{r['method']} & {r['evals']} & {r['kde_ise']} & "
            f"{r['kde_time']} & & {r['nw_mse']} & {r['nw_time']} \\\\\n"
        )
...
\vspace{0.5em}
\footnotesize
Values are means (standard errors) over 20 replicates. KDE: bimodal DGP, """
        r"""Gaussian kernel, $n=200$. NW: $\sigma=1.0$, Gaussian kernel, $n=200$.
\end{table}
"""
    )
    return latex
...
"""
    for r in rows:
        latex += (
            f"{r['n']} & {r['kde_grid']} & {r['kde_newton']} & {r['kde_speedup']} & "
            f"{r['nw_grid']} & {r['nw_newton']} & {r['nw_speedup']} \\\\\n"
        )
...
def table_mv_nw(mv_nw_df: pd.DataFrame) -> str:
    ...
    latex = (
        r"""\begin{table}[t]
\centering
\caption{Multivariate NW regression: Newton vs Grid search by dimension """
        r"""($n=300$, Gaussian kernel).}
\label{tab:mv-nw}
...
"""
    ...
    for r in rows:
        latex += (
            f"{r['d']} & {r['grid_mse']} & {r['newton_mse']} & "
            f"{r['grid_time']} & {r['newton_time']} & {r['speedup']} \\\\\n"
        )
...
\vspace{0.5em}
\footnotesize
Regression function: $m(\mathbf{x}) = \sin(x_1) + 0.5 x_2$. """
        r"""Values are means over 20 replicates.
\end{table}
"""
    )
    return latex
...
def robustness_summary(kde_df: pd.DataFrame, nw_df: pd.DataFrame) -> str:
    ...
    nw_ratio = (
        f"mean={np.mean(nw_ratios):.3f}, "
        f"range=[{min(nw_ratios):.3f}, {max(nw_ratios):.3f}]"
    )
    nw_speedup = (
        f"mean={np.mean(nw_speedups):.2f}x, "
        f"range=[{min(nw_speedups):.2f}x, {max(nw_speedups):.2f}x]"
    )
    worst_ratio_pct = f"{(max(max(ratios), max(nw_ratios)) - 1) * 100:.0f}"
    nw_mean_speedup = f"{np.mean(nw_speedups):.1f}x average speedup."
...
KDE (across {len(kernels)} kernels and {len(dgps)} DGPs):
  - Newton/Grid ISE ratio: {kde_ratio}
  - Grid/Newton speedup: {kde_speedup}

NW Regression (across {len(kernels)} kernels and 3 noise levels):
  - Newton/Grid MSE ratio: {nw_ratio}
  - Grid/Newton speedup: {nw_speedup}
...
skewed, heavy-tailed). Newton achieved ISE/MSE within {worst_ratio_pct}% of Grid search
across all configurations. For NW regression, Newton provided {nw_mean_speedup}"
"""
...
def print_plain_text_tables(
    ...
    print(
        f"{'Method':<12} {'Evals':>6} {'KDE ISE (x10⁻³)':>18} "
        f"{'KDE Time':>10} {'NW MSE':>14} {'NW Time':>10}"
    )
    print("-" * 70)
    ...
    for method in ["Grid", "Golden", "Newton", "Silverman"]:
        ...
        print(
            f"{method:<12} {evals:>6} {kde_ise_mean:>8.2f} ({kde_ise_se:.2f}) "
            f"{kde_time:>10.1f} {nw_mse_mean:>7.3f} ({nw_mse_se:.3f}) "
            f"{nw_time:>10.1f}"
        )
    ...
    print("=" * 70)
    print(
        f"{'n':>5} {'KDE Grid':>10} {'KDE Newton':>12} {'Speedup':>10} "
        f"{'NW Grid':>10} {'NW Newton':>12} {'Speedup':>10}"
    )
    print("-" * 70)
    ...
    for n in [100, 200, 500]:
        ...
        print(
            f"{n:>5} {kde_grid:>10.1f} {kde_newton:>12.1f} "
            f"{kde_grid / kde_newton:>9.2f}x {nw_grid:>10.1f} "
            f"{nw_newton:>12.1f} {nw_grid / nw_newton:>9.2f}x"
        )
    ...
    for n in [100, 200, 500]:
        ...
        print(
            f"{n:>5} {kde_grid:>10.2f} {kde_newton:>12.2f} "
            f"{kde_grid / kde_newton:>9.2f}x {nw_grid:>10.2f} "
            f"{nw_newton:>12.2f} {nw_grid / nw_newton if nw_newton else 0:>9.2f}x"
        )
...
def main() -> None:
    """Generate all tables."""
    print("Loading data...")
    kde_df, nw_df, boot_df, _mv_df, boot_nw_df, mv_nw_df = load_data()
